...
This guide describes how to automate the setup of your development environment in a brand-new container using `ansible-pull`.

## Overview

The setup process is split into two phases:
1.  **Bootstrap Phase:** A shell script installs minimal dependencies (Git, Python, Ansible).
2.  **Automation Phase:** `ansible-pull` clones this repository and executes `playbooks/dev_env_setup.yml` with the `cli` tag.

## Prerequisites

1.  **Internet Access:** To clone the repository and install packages.
...
| Task | Handled By |
| :--- | :--- |
| **Prerequisites** (Git, Ansible) | `scripts/bootstrap_container.sh` |
...
- **Permission Denied:** The script must run as a user with `sudo` privileges or as `root` (standard for containers).
